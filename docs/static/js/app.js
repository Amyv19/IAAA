...
function getInputValue(model, column, formData) {
  const inputKey = model.input_map[column];
  if (inputKey && formData[inputKey] !== undefined) return formData[inputKey];
  return model.template[column];
}
...
async function loadBrowserModel() {
  if (!browserModelPromise) {
    browserModelPromise = fetch(ROUTES.model).then(res => {
      if (!res.ok) throw new Error('Static model unavailable');
      return res.json();
    });
  }
...
function predictWithBrowserModel(model, formData) {
  let coefIndex = 0;
  let prediction = model.intercept;

  model.numeric.columns.forEach((column, index) => {
    let value = asNumber(getInputValue(model, column, formData));
    if (value === null) value = model.numeric.imputer_statistics[index];
    const scaled = (value - model.numeric.scaler_mean[index]) / model.numeric.scaler_scale[index];
    prediction += scaled * model.coefficients[coefIndex];
    coefIndex += 1;
  });

  model.categorical.columns.forEach((column, index) => {
    let value = getInputValue(model, column, formData);
    if (value === null || value === undefined || value === '') {
      value = model.categorical.imputer_statistics[index];
    }
    value = String(value);

    model.categorical.categories[index].forEach(category => {
      prediction += (value === String(category) ? 1 : 0) * model.coefficients[coefIndex];
      coefIndex += 1;
    });
  });
...
async function predictPrice(formData) {
  try {
    ...
    return predictWithBrowserModel(model, formData);
  } catch (modelError) {
    const res = await fetch(ROUTES.predict, {
      method: 'POST',
...
document.getElementById('predict-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    document.getElementById('price-result').textContent =
      '$' + prediction.toLocaleString('es-MX', { minimumFractionDigits: 2 });
    document.getElementById('result-box').classList.remove('hidden');
  } catch (error) {
    alert('Error al calcular la prediccion: ' + error.message);
  } finally {
    btn.disabled = false;
...
Promise.all([
  fetch(ROUTES.mapData).then(r => r.json()),
